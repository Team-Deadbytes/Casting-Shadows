        ...
        void DrawControlPanelSpportFor3RdPartyAssets(float width)
        {
            ...
            else
            {
                QUI.BeginHorizontal(width);
                {
                    ...
                    {
                        Application.OpenURL("https://assetstore.unity.com/packages/tools/visual-scripting/playmaker-368");
                    }
                }
                QUI.EndHorizontal();
                #endregion
            ...
            else
            {
                ...
                if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonMasterAudioEnabled), 200, 40))
                {
                    ...
                    if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonExternalLink), 40, 40))
                    {
                        Application.OpenURL("https://assetstore.unity.com/packages/tools/audio/master-audio-aaa-sound-5607");
                    }
                }
                QUI.EndHorizontal();
            }
            ...
            else
            {
                QUI.BeginHorizontal(width);
                {
                    ...
                    if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonExternalLink), 40, 40))
                    {
                        Application.OpenURL("https://assetstore.unity.com/packages/tools/gui/energy-bar-toolkit-7515");
                    }
                }
                QUI.EndHorizontal();
            }
            ...
            else
            {
                ...
                {
#if dUI_TextMeshPro
                    if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonTextMeshProEnabled), 200, 40))
                    {
                        NotificationWindow.YesNo("Disable support for TextMesh Pro?",
                                                 "This will remove '" + DUI.SYMBOL_TEXT_MESH_PRO + "' from Scripting Define Symbols in Player Settings.",
                                                 () => { QUtils.RemoveScriptingDefineSymbol(DUI.SYMBOL_TEXT_MESH_PRO); },
                                                 null);
                    }
#else
                    if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonTextMeshProDisabled), 200, 40))
                    {
                        NotificationWindow.YesNo("Enable support for TextMesh Pro?",
                                                "Enable this only if you have TextMesh Pro already installed." +
                                                "\n\n" +
                                                "This will add '" + DUI.SYMBOL_TEXT_MESH_PRO + "' to Scripting Define Symbols in Player Settings.",
                                                () => { QUtils.AddScriptingDefineSymbol(DUI.SYMBOL_TEXT_MESH_PRO); },
                                                null);
                    }
#endif
                    QUI.Space(2);
                    if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonExternalLink), 40, 40))
                    {
                        Application.OpenURL("https://assetstore.unity.com/packages/essentials/beta-projects/textmesh-pro-84126");
                    }
                }
                QUI.EndHorizontal();
            }
        ...
        void DrawControlPanelDoozyModules(float width)
        {
            ...
            else
            {
                QUI.BeginHorizontal(width);
                {
                     ...
                     if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonSceneLoaderDisabled), 242, 40))
                     {
                           NotificationWindow.YesNo("Enable the automated Scene Loader?",
                                                    "This will remove '" + DUI.SYMBOL_SCENE_LOADER + "' from Scripting Define Symbols in Player Settings.",
                                                    () => { QUtils.RemoveScriptingDefineSymbol(DUI.SYMBOL_SCENE_LOADER); },
                                                    null);
                     }
#else
                    if (QUI.Button(DUIStyles.GetStyle(DUIStyles.ControlPanel.PageControlPanelButtonSceneLoaderEnabled), 242, 40))
                    {
                        NotificationWindow.YesNo("Disable the automated Scene Loader?",
                                                 "Do this if you do not want to use the Scene Loader and the UIManager will no longer add it to the scene automatically." +
                                                 "\n\n" +
                                                 "This will add '" + DUI.SYMBOL_SCENE_LOADER + "' to Scripting Define Symbols in Player Settings.",
                                                 () => { QUtils.AddScriptingDefineSymbol(DUI.SYMBOL_SCENE_LOADER); },
                                                 null);
                    }
#endif
                }
