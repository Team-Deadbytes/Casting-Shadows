...
namespace DoozyUI
{
    ...
    public class UIElementEditor : QEditor
    {
        ...
        void DrawSettings()
        {
            ...
            QUI.BeginHorizontal(WIDTH_420);
            {
                QUI.PropertyField(autoHide, 12);
                QUI.Label("auto hide", DUIStyles.GetStyle(DUIStyles.TextStyle.LabelNormal), 56);
                showAutoHideDelay.target = autoHide.boolValue;
                if (showAutoHideDelay.faded > 0.05f)
                {
                    QUI.Space(-5);
                    QUI.Label("after a", DUIStyles.GetStyle(DUIStyles.TextStyle.LabelNormal), 40 * showAutoHideDelay.faded);
                    QUI.PropertyField(autoHideDelay, 40 * showAutoHideDelay.faded);
                    QUI.Label("second" + (autoHideDelay.floatValue == 1 ? "" : "s") + " delay", DUIStyles.GetStyle(DUIStyles.TextStyle.LabelNormal), 84 * showAutoHideDelay.faded);
                }
            }
            QUI.EndHorizontal();
            QUI.Space(SPACE_4);
