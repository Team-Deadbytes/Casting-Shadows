...
namespace DoozyUI
{
    ...
    public partial class DUI
    {
        ...
        public static void RefreshUIElementsDatabase()
        {
            ...
            if (UIElementsDatabase == null) { UIElementsDatabase = new Dictionary<string, NamesDatabase>(); }
            UIElementsDatabase.Clear();
            fileNames = null;
            ...
            {
                EditorUtility.DisplayProgressBar("Refreshing UIElements Database", fileNames[fileIndex], ((fileIndex + 1) / (fileNamesLength + 2)));
        ...
        {
            if (m_UIElementCategories == null) { m_UIElementCategories = new List<string>(); }
            m_UIElementCategories.Clear();
            m_UIElementCategories.Add(CUSTOM_NAME);
        ...
        private static void ValidateUIElementsDatabase()
        {
            ...
            if (UIElementsDatabase == null) { RefreshUIElementsDatabase(); return; }
            if (!UIElementCategoryExists(DEFAULT_CATEGORY_NAME))
            {
                refreshDatabase = true;
                NamesDatabase newCategory = CreateAsset<NamesDatabase>(RELATIVE_PATH_UIELEMENTS, DEFAULT_CATEGORY_NAME);
                newCategory.Init();
                EditorUtility.SetDirty(newCategory);
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();
        ...
        private static void ValidateUIButtonsDatabase()
        {
            ...
            {
                refreshDatabase = true;
                NamesDatabase newCategory = CreateAsset<NamesDatabase>(RELATIVE_PATH_UIBUTTONS, DEFAULT_CATEGORY_NAME);
                newCategory.Init();
                EditorUtility.SetDirty(newCategory);
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();
